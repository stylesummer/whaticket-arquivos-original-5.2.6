...
if (choosenQueue.options.length === 0) {
  ...
  if (queue.outOfHoursMessage !== null && queue.outOfHoursMessage !== "" && !isNil(schedule)) {
    ...
    const startTimePM = schedule.startTimePM ? moment(schedule.startTimePM, "HH:mm") : null;
    const endTimePM = schedule.endTimePM ? moment(schedule.endTimePM, "HH:mm") : null;
    const isWithinBusinessHours = (now.isBetween(startTime, endTime, null, '[]') || (startTimePM && endTimePM && now.isBetween(startTimePM, endTimePM, null, '[]')));
      ...
      if (!msg.key.fromMe && scheduleType && ticket.status !== "open") {
        ...
        if (scheduleType.value === "queue" && ticket.queueId !== null) {
          ...
          if (
            ...
            const startTime = moment(schedule.startTime, "HH:mm");
            const endTime = moment(schedule.endTime, "HH:mm");
            const startTimePM = moment(schedule.startTimePM, "HH:mm");
            const endTimePM = moment(schedule.endTimePM, "HH:mm");
			
            if (now.isBefore(startTime) || now.isAfter(endTime) && (now.isBefore(startTimePM) || now.isAfter(endTimePM))) {
